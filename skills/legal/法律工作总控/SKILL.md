...
**开场声明与确认门（强制）：同一会话首次调用本 Skill 时，必须先原样完整输出以下声明块，再取得用户确认。**
> **声明**
>
...
1. 声明必须作为独立的用户可见内容呈现，不得埋入或合并到 `commentary`、工作进度、`summary`、总结或工具输出中。
2. 输出声明后，如运行环境提供 `mcp__rlcd__ask`，立即调用该工具显示 question box：
   - Prompt：`Continue with the legal workflow?`
   - Options：`Continue`、`Cancel`
3. 用户选择 `Continue` 后方可读取案件材料、检索法规、起草文书或执行外部写入；选择 `Cancel` 时立即停止法律工作。
4. `mcp__rlcd__ask` 不可用、被拒绝或超时时，在最终答复中原样完整输出声明，并以普通文字询问是否确认继续，然后停止当前轮次。
5. 为防止前置消息被界面折叠，通过 question box 确认后，该次法律任务的首个最终答复仍须在正文开头原样完整保留上述声明块，不得仅依赖先前的 `commentary`。
6. 同一会话确认一次即可；后续法律工作无需重复确认。新会话重新执行本确认门。
...
输出计划或拦截说明中必须明确出现 `当前事项.md`、`成果类型分类`、`法律文书出稿前审查`、对应链路的核心输入文件和 `health_check.py`；涉及正式法律意见、客户报告、法院材料时还必须出现 `来源边界记录`；不得只笼统写“出稿检查”。
...
- 涉及管辖权异议、移送管辖、协议管辖、专属管辖、上诉审查范围等民事诉讼程序问题时，优先点名路由 `民事一审诉讼`，再按需要联动 `法规案例检索` 或 `诉讼文书起草`。
...
- 凡最终产物是本地 `.docx`，必须在业务子 Skill 生成正文、结构化数据或语义 HTML 后，先路由 `法律文书出稿前审查`；通过后再路由 `法律文书模板与导出` 完成最终 Word 排版和导出。已在模板克隆 manifest 登记的起诉状案由，必须走 DOCX 母版克隆填充链路，不得退回传统段落式起诉状或普通 HTML 表格重制格式。
