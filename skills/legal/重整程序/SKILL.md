---
name: 重整程序
description: 破产重整程序全流程管理。覆盖重整申请与受理、重整期间管理、重整计划草案编制、债权分组表决、法院强制批准、重整计划执行与监督等全部环节。适用于管理人（R1）、债权人代理（R2）、债务人代理（R3）三类角色。当用户发送客户编号并提及"重整"、"重整计划"、"重整程序"等指令时触发使用。
---
...
本 Skill 只处理「重整程序」专业任务；案件隔离、事项路径、文件读取、OCR 复查、来源披露、缺口归档、法规/案例/Wiki 核验和复盘台账更新均按法律工作总控共享协议执行。
...
- 用户明确提到「重整程序」或本 Skill frontmatter 描述中的任务。
- 用户请求生成、审查、分析、计算、管理或推进与「重整程序」对应的法律工作成果。
