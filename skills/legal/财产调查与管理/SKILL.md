---
name: 财产调查与管理
description: R1（管理人）专属Skill，覆盖破产程序中的财产全面调查、可撤销/无效行为追回、财产管理方案编制。当用户发送客户编号并提及"财产调查"、"财产清查"、"可撤销行为"、"追回财产"、"财产管理方案"等指令时触发使用。
version: 1.0.0
category: 破产程序
...
本 Skill 只处理「财产调查与管理」专业任务；案件隔离、事项路径、文件读取、OCR 复查、来源披露、缺口归档、法规/案例/Wiki 核验和复盘台账更新均按法律工作总控共享协议执行。
...
- 用户明确提到「财产调查与管理」或本 Skill frontmatter 描述中的任务。
- 用户请求生成、审查、分析、计算、管理或推进与「财产调查与管理」对应的法律工作成果。
